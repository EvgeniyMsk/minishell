...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/09/08 20:56:25 by qsymond           #+#    #+#             */
/*   Updated: 2020/09/08 21:22:04 by qsymond          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		ft_start_splitter(char **str, char const *s, char c)
{
	size_t	j;
	int		q;
	int		dq;
...
{
	char			**str;
	const size_t	countw = ft_countw(s, c);
	...
	if (!s || !(str = (char **)malloc(sizeof(char *) * (countw + 1))))
		return (NULL);
	while (g_i < countw)
		if (ft_start_splitter(str, s, c) == -1)
			return (NULL);
	str[g_i] = NULL;
	return (str);
}
